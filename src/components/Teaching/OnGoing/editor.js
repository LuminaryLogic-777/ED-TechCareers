...
const CodeEditor = ({
  ...
  const deleteCode = async (qId, compilerId) => {
    ...
    try {
      const response = await fetch(
        `${ApiBaseURL}mentor-management/inst-practice-delete-code/`,
        {
          method: "POST",
          headers: {
            Authorization: `Token ${JSON.parse(loggedIn).token}`,
  ...
  const getCompilerData = async (main_data_id, buttonClicked) => {
    try {
      const result = await fetch(
        `${ApiBaseURL}mentor-management/inst-practice-question-submission-responce/?main_data_id=${main_data_id}`,
        {
          method: "GET",
          headers: {
            Authorization: `Token ${JSON.parse(loggedIn).token}`,
      ...
      if (buttonClicked !== "Run") {
        fetchProblemData();
        setIsRun("Submit");
        navigate("/mentor/teaching/" + selectedQs + "/submission");
