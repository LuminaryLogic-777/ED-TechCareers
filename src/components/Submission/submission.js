...
const Submission = () => {
  ...
  const handleDrop = (e) => {
    ...
    const droppedCheckboxes = droppedFiles.map((file, index) => ({
      id: file.name + index,
      label: file.name,
      size: file.size,
      file: file,
  ...
  const handleWeekSelect = (week, value) => {
    const topicsForSelected = weekData[selectedWeek];
  ...
  // console.log("search", weekData);

  useEffect(() => {
    if (selectedWeek && weekData[selectedWeek]) {
      setTopicsForSelectedWeek(weekData[selectedWeek]);
      setIsDoneDisabled(!selectedWeek || !selectedTopic);
      // console.log("week dta", selectedWeek);
    }
  }, [selectedWeek, weekData, selectedTopic]);
  ...
  const handleDoneClick = () => {
    if (selectedWeek) {
      ...
      alert("Please select both week and topic.");
    }
    handleStartUpload();
          ...
          setTimeout(() => {
            ...
            if (currentProgress === 100) {
              const updatedCheckboxes = checkboxes.map((cb) =>
                cb.id === file.id ? { ...cb, isChecked: false } : cb
              );
              setCheckboxes(updatedCheckboxes);
            }
          }, 3000);
    ...
    if (loggedIn) {
      if (selectedWeek) {
        try {
          ...
          if (
            selectedFiles.some(
              (file) => file.selectedWeekData == null || file.id == null
            )
          ) {
            alert(
          ...
          selectedFiles.forEach((file, index) => {
            ...
            formData.append(
              `course_submission_id_${index + 1}`,
              file.selectedWeekData
            );
          });
      ...
      <div className="flex flex-wrap -mx-3 gap-y-4">
        <div className="xl:w-[calc(100%-320px)] lg:w-[calc(100%-260px)] w-full px-3">
          ...
          <div className="p-5 rounded-2xl shadow-[0px_5px_25px_0px_rgba(59,154,168,0.15)] bg-white">
            <h2 className="text-gray-600 text-xl font-semibold font-['Outfit']">
              Upload File
            </h2>
            <div className="flex flex-wrap -mx-2 gap-y-4">
                ...
                <div class="flex items-center justify-center w-full h-full">
                  ...
                  >
                    <div class="flex flex-col items-center justify-center pt-5 pb-6">
                      <div className="w-[60px] h-[60px] bg-sky-500 bg-opacity-20 rounded-full flex justify-center items-center">
              ...
              <div className="information md:w-[220px] w-full px-2">
                <div className="p-3 bg-slate-50 rounded-[10px] border-2 border-slate-300 h-full">
                  <p>1.You can upload only .PDF files</p>
                  <br></br>
                  <p> 2.The size of the PDF’s file must be less than 10 mb</p>
                  <br></br>
                  <p> 3.Select the file and click upload button.</p>
                </div>
              </div>
            </div>
            <div>
                ...
                {checkboxes.map((checkbox) => (
                  <div className="p-2 bg-[#F8FFFD] rounded-[10px] border border-[#C5D8DB] mb-3">
                    ...
                    >
                      ...
                      {uploadProgress < 100 && (
                        <button
                          className={`px-3 py-[7px] bg-opacity-20 rounded-[5px] justify-center items-center gap-2.5 inline-flex text-sm font-normal duration-150 sm:ms-2 ms-auto whitespace-nowrap ${
                ...
                <div className="overflow-x-auto mb-3">
                  {searchResults && searchResults.data ? (
                    <table className="w-full border-collapse border-slate-400 mb-3">
                      <thead>
                        <tr>
                          <th className="p-3 text-start bg-slate-200 border-slate-300 text-sm font-semibold text-gray-600 rounded-l-md">
                            Sl.No
                          </th>
                          <th className="p-3 text-start bg-slate-200 border-slate-300 text-sm font-semibold text-gray-600">
                            File Name
                          </th>
                          <th className="p-3 text-start bg-slate-200 border-slate-300 text-sm font-semibold text-gray-600">
                            Week
                          ...
                          <th className="p-3 text-start bg-slate-200 border-slate-300 text-sm font-semibold text-gray-600 rounded-r-md">
                            Uploaded Date & Time
                          </th>
                          <th className="p-3 text-start bg-slate-200 border-slate-300 text-sm font-semibold text-gray-600 rounded-r-md">
                            Status
