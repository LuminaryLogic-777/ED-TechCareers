...
const MentorProfileCard = () => {
    ...
    if (loggedIn) {
      fetch(`${ApiBaseURL}mentor-management/inst-profileinfo/`, {
        ...
        .then((data) => {
          ...
          if (message === 1) {
            setUserProfile({
              firstName: Instructor_f_nm || "",
              lastName: Instructor_l_nm || "",
              email: Instructor_email || "",
              phoneNum: Instructor_ph_no || "",
              profilePic: Instructor_profile_pic || null,
              Instruct_course_details: Inst_course_details || [],
            });
          } else {
  ...
  const handlePasswordChange = () => {
    ...
    fetch(`${ApiBaseURL}mentor-management/inst-profilepasswordupdate/`, {
      method: "POST",
      headers: {
  ...
  const handleSaveEditedPhoneNum = (e) => {
    ...
    fetch(`${ApiBaseURL}mentor-management/inst-profiledetailsupdate/`, {
      method: "POST",
      headers: {
        Authorization: `Token ${JSON.parse(loggedIn).token}`,
      },
      ...
      <div className="student-wrapper py-4">
        <div className="flex flex-wrap -mx-3">
          ...
          <div className="w-full px-3">
            {userProfile.Instruct_course_details.map((course, index) => (
              <div
                key={index}
                  ...
                  <div class="md:w-8/12 w-full px-3">
                    <div className="flex mb-2">
                      ...
                      <span className="value text-gray-700 text-base font-medium ml-2">
                        Batch-{course.batch_id} {course.course}
                      </span>
                    </div>
                    ...
                    <div className="flex mb-2">
                      ...
                      <div className="value text-gray-700 text-base font-medium ml-2">
                        {course &&
                          ...
                          course.inst_project_assign.map((batch) => (
                            <div key={batch.batch_id}>
                              <span className="text-gray-700 text-base font-medium">
                                {batch.project_topic && batch.project_title
                                  ? `${batch.project_topic} [${batch.project_title}]`
                                  : "N/A"}
                              </span>
                            </div>
