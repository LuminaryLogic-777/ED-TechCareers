    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `${ApiBaseURL}user-management/notice/`,
          {
            headers: {
              Authorization: `Token ${JSON.parse(loggedIn).token}`,
            },
    ...
    const fetchData = async () => {
      try {
        if (loggedIn) {
          const token = JSON.parse(loggedIn).token;
          const response = await fetch(
            `${ApiBaseURL}test-management/std-all-contests/`,
            {
              method: "GET",
              headers: {
                Authorization: `Token ${token}`,
          ...
          Object.values({
            main_data1: data.main_data1.quiz_test_details,
            main_data2: data.main_data2.mock_test_details,
            main_data3: data.main_data3.practice_test_details,
          }).forEach((testDetails) => {
            testDetails.forEach((item) => {
              if (item.contest_details) {
                item.contest_details.forEach((contest) => {
                  let type = "";
                  let color = "";
                  let title = "";
                  ...
                  resultArray.push({
                    ...
                    details: contest,
                    next_q_id: item?.last_info?.question__id,
                    next_q_week: item?.last_info?.question__week,
          ...
          resultArray.sort((a, b) =>
            a.details.course_name_value.localeCompare(
              b.details.course_name_value
            )
          );
          // console.log("Resulted Array", resultArray);

          setOtherQuiz(data.main_data1.quiz_test_details);
          setOtherMock(data.main_data2.mock_test_details);
          setOtherPractice(data.main_data3.practice_test_details);

          setOtherCardsData(resultArray);
              ...
              <div className="other-cards">
                ...
                {/* <div className="other-cards-container">
                  {otherCards(
                    "blue",
                    "Practice Week",
                    practiceContestData[0],
                    seletecedPracticeStatus
                  )}
                  {otherCards(
                    "cyan",
                    "Quiz Week",
                ...
                <div className="other-cards-container">
                  <Slider {...carouselSettings}>
                    {/* {console.log(otherCardsData.length)} */}
                        ...
                        <>
                          {card && card.details !== undefined ? (
                            otherCards(
                              ...
                              card.next_q_week,
                              card.course_id,
                              card.details.week_value
                            )
                          ) : (
                            <div key={cardIndex}>No data found</div>
                          )}
                        </>
                      </div>
                    ))}
