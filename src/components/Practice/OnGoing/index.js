...
const OnGoing = ({ courseId, week, selectedQsId }) => {
  ...
  const {setSelectedWeekIdForPractice,selectedWeekIdForPractice,setSelectedQsIdForPractice,selectedQsIdForPractice} =
  ...
  //                     "/student/practice/" + step?.q_id + "/problem";
  //                   setTagDisable(true);
  //                 }}
    ...
    return (
      <div className="stepper-box">
        {steps.map((step) => {
          ...
          // console.log(`Step ${step.title} - que_status: ${step.que_status}`);  
  
          // Set the class to "SubmitAns" if que_status is true, otherwise default to "steper-topic-active"
          const activeClass = step.que_status ? "submitAns" : "steper-topic-active";
          // const activeClass = step.isOnGoing ? "steper-topic-active" : step.que_status?"submitAns":"steper-topic";
          ...
          return (
            <div className="step-item" key={step.q_id}>
              ...
              <div className="content">
                <a
                  className={selectedQs === step?.q_id ? activeClass : step.que_status ? "submitAns" : ""}
                  onClick={() => {
                    window.location.href = "/student/practice/" + step?.q_id + "/Problem";
                    setTagDisable(true);
                  }}
                  href="javascript:void(0)"
                >
  ...
  useEffect(() => {
    const fetchFakeData = async () => {
      if (loggedIn) {
        ...
        setSelectedQsIdForPractice(selectedQs);
    ...
    fetchFakeData();
  }, [loggedIn, selectedWeekIdForPractice,selectedQsIdForPractice,selectedQs, week]);
