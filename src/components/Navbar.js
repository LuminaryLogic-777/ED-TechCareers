...
const Navbar = () => {
  ...
  const [profile, setProfile] = useState(user); // TODO: remove profil and setProfile and only use 'user'
  ...
  const [openNoBatchModal, setOpenNoBatchModal] = useState(false);
  ...
  const checkUserBatch = async () => {
    if (loggedIn) {
      try {
        ...
        if (response.data.message === "You have No Batch") {
          setOpenNoBatchModal(true);
        ...
        // console.error("Error checking user batch:", error);
      }
    }
  };
  ...
  //   try {
  //     const response = await axios.get(
  //       "https://devdjango.maangcareers.com/test-management/user-valid-check/",
  //       {
  //         headers: {
  //           Authorization: "Token " + user.token,
  //         },
  ...
  //     if (response.data.message === "You have No Batch") {
  //       // Show popup modal indicating the user doesn't have a course
  //       // Example: setOpenNoBatchModal(true);
  //       console.log("You don't have a course");
  //     } else {
  //       // Continue with the normal flow
  //       console.log("You have a course");
  //       window.location.href = "/student"; // or any other logic you want to implement
  //     }
  //   } catch (error) {
  //     console.error("Error checking user batch:", error);
  //   }
  ...
  return (
    <div>
      ...
      <div className="hidden lg:block py-5 bg-theme ">
        <div className="2xl:container mx-auto 2xl:px-5 lg:px-24 sm:px-12 px-10">
          <div className="lg:flex justify-between items-center">
            <img
              ...
              alt="Logo"
            />
            <ul className="flex font-base xl:space-x-10 space-x-4 text-md text-white">
              <li
                className={
                  window.location.pathname === "/"
                    ? "text-primary font-semibold cursor-pointer"
            ...
            {Object.keys(user).length <= 1 ? (
              <div className="space-x-5">
                <button
                  ...
                  className="font-base font-medium text-md px-5 py-1 bg-primary border-2 border-primary text-white rounded-md hover:bg-primaryDark transition duration-500"
                >
                  Explore Courses
                </button>
                <button
                  className="font-base font-medium text-md border-2 border-white text-white px-5 py-1 rounded-md hover:bg-white hover:text-theme hover:font-semibold transition ease-in-out duration-500"
      ...
      <div className="2xl:container mx-auto lg:px-24 sm:px-12 py-5 px-10 flex lg:hidden justify-between items-center bg-theme">
        <img
          src="/logos/Logo_t_white_64.png"
          onClick={() => (window.location.href = "/")}
          className="w-28 cursor-pointer"
        ...
        <div className="flex space-x-3">
          {Object.keys(user).length <= 1 ? (
            <button
              className="font-base font-medium border-2 border-white text-white px-2 py-1 rounded-md hover:bg-white hover:text-theme hover:font-semibold transition ease-in-out duration-250"
              onClick={() => (window.location.href = "/login")}
              style={{ fontSize: "12px" }}
            >
              Login / Signup
          ...
          <Modal
            open={openNoBatchModal}
            onClose={() => setOpenNoBatchModal(false)}
            aria-labelledby="modal-modal-title"
            aria-describedby="modal-modal-description"
          >
            <div className="h-screen flex justify-center items-center">
              <div className="xl:w-[700px] md:w-[500px] w-[340px] px-10 py-16 bg-white rounded-lg shadow-xl text-center relative">
                <button
                  onClick={() => setOpenNoBatchModal(false)}
                ...
                <span className="mb-20">
                  To continue, please check out the courses to access the
                  student portal
                </span>
                <br />
                <a
                  href="/courses"
                  className="inline-block py-3 px-6 mt-5 font-base text-xl font-medium text-white rounded bg-till-600 hover:bg-till-900 duration-150"
                >
                  Checkout{" "}
                  <i className="bx bxs-right-arrow-alt align-middle"></i>
                </a>
              </div>
            </div>
          </Modal>
